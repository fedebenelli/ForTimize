module ForTimize__gradient_free_nelder_mead
   use ForTimize__constants, only: pr
   use ForTimize__base, only: Optimizer
   use ForTimize__base, only: objective_function
   implicit none
   ...
   procedure(objective_function), pointer :: obj_fun
...
contains
   ...
   subroutine optimize(self, foo, X, F, data)
      ...
      procedure(objective_function) :: foo !! Objective function
...
end module ForTimize__gradient_free_nelder_mead
